...
- Use `logger.*` from `api/lib/logger.js` for all diagnostic output — never `console.warn/log/error` in production code. In GitHub Actions, `logger.warn` routes through `core.warning()` which emits workflow annotations; `console.*` does not.
