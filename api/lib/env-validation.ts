 ...
 *
 * Returns undefined if the input is undefined or results in an empty
 * string after normalization. When normalization changes the value and
 * a variable name is provided, emits a warning through the logger.
 *
 * Shared by the GitHub App bootstrap path and re-exported for LLM
 * callers that previously imported it from api/lib/llm/env.ts.
 */
...
/**
 * Read and normalize a single environment variable by name.
 */
 ...
 * Whitespace-only and quote-only values are treated as unset.
 */
 ...
 * Returns the normalized value: whitespace trimmed, surrounding quotes stripped.
 * Empty or whitespace-only values are treated as unset.
 */
export function getPrivateKey(): string | undefined {
  for (const key of PRIVATE_KEY_VARS) {
    const val = getNormalizedEnv(key);
    if (val) return val;
  }
  return undefined;
}
...
export function validateEnv(requireWebhookSecret = false): EnvValidationResult {
  ...
  // Check required vars (normalize before presence check so whitespace-only values fail)
