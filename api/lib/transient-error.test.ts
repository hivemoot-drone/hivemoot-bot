...
describe("isAutoMergeNotAllowedError", () => {
  ...
  it("returns false for a plain Error with the old string sentinel", () => {
    // The old msg.includes("PullRequestAutoMergeNotAllowed") check would have failed too —
    // the real GitHub error is a GraphqlResponseError, not a plain Error.
  ...
  it("returns false for a GraphqlResponseError with FORBIDDEN type but unrelated message", () => {
    const err = new GraphqlResponseError(
      { url: "https://api.github.com/graphql" },
      {},
      { data: null, errors: [{ message: "Resource not accessible.", type: "FORBIDDEN" }] }
    );
    expect(isAutoMergeNotAllowedError(err)).toBe(false);
  });

  it("returns false for a GraphqlResponseError with UNPROCESSABLE type", () => {
    const err = new GraphqlResponseError(
      { url: "https://api.github.com/graphql" },
      {},
  ...
  it("returns false for a plain object without name", () => {
    expect(isAutoMergeNotAllowedError({ message: "PullRequestAutoMergeNotAllowed" })).toBe(false);
  });
