...
import { buildNotificationComment, buildAutomergeStatusComment, NOTIFICATION_TYPES } from "./bot-comments.js";
...
describe("PROperations", () => {
  ...
  describe("upsertAutomergeStatus", () => {
    ...
    it("skips write when eligible and reason are unchanged", async () => {
    ...
    it("updates comment when eligible state changes from ineligible to eligible", async () => {
      const existingBody = buildAutomergeStatusComment(testRef.prNumber, false, "CI not passing");
      vi.mocked(mockClient.rest.issues.listComments).mockResolvedValue({
        data: [
          {
    ...
    it("ignores automerge-status comments from other apps", async () => {
      ...
      // Treats foreign comment as non-existent → creates a new one
